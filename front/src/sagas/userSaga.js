import axios from "axios";
import { takeLatest,call, put } from "redux-saga/effects";
import {user_login_request} from '../reducers/user'

async function loginAPI(payload){
    const result = await axios.post('http://localhost:3500/user/login',payload)
    const {token} = result.data
    console.log(token)
    const response = await axios.post('http://localhost:3500/user/me',null,{
        headers:{
            "Authorization":`Bearer ${token}`
        }
    })
    return response
}


function* login(action){
    //axios 관련 코드를 넣을게여.
    try{const result = yield call(loginAPI,action.payload)
    //const result = axios.post('http://localhost:3500/user/login',action.payload)
    // console.log(result)
        yield put({
            type:'USER/LOGIN_SUCCESS',
            payload:result.data
        })
    }catch(e){
        yield put({
            type:'USER/LOGIN_FAIL',
            payload:e.response.data
        })
    }
}

export default function* userSaga(){
    // LOGIN/USER_REQUEST 라는 액션값을 추적할거에여
    // takeLatest / Side effects
    yield takeLatest(user_login_request.toString(),login)

}